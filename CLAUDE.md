...
sounds/ is split into two top-level siblings — **main/** for the shared engine assets (characters, equipments/shields, equipments/weapons, keyboards, menus, misc) and **builder/** for the per-entity map-object assets (kombat/npc, kombat/projectiles, construction, transitions, transportation, traps, zones, audio, interaction, misc). There is no per-pack indirection layer — the previous `sounds/<pack>/` selector (and the matching `soundpack` global) was removed when it became clear nobody was swapping packs and the extra path layer was just complicating every authored path. Players who want to customize audio still do so by dropping clips into the relevant per-entity subfolder (a character theme, a keyboard theme, custom NPC clips), which works exactly as before, just one directory shallower.
...
- equipments/shields/<name>/data/info.sif — 43 shields. key=value per line. Fields: defence, wear mode (0/1), weight, shield strength, unlock level. Clips live in the sibling general/ subfolder: draw.ogg, wear.ogg, remove.ogg, hit.ogg, break1.ogg, break2.ogg.
...
Sound assets are looked up via get_pack_sound("...") / get_map_sound("...") with glob patterns like main/characters/<chartype>/map/*camclear* or builder/construction/walls/<wallname>/*death*. The chartype, menutype, keyboardtheme strings drive theme selection within sounds/main/.
...
1. **Compile** — runs nvgt -c -Q sf.nvgt directly. The version flow goes the other direction now: includes/version.nvgt is the source of truth (dev-edited), and sf.nvgt's main() syncs build/version.txt from it on every uncompiled launch (gated on `!SCRIPT_COMPILED` and skipped when the value already matches). tools.py just reads the now-synced build/version.txt to compute the release tag (V<version>0). The compiled cst/ output is moved into releases/windows/<Game>_windows_portable_password_is_<password>/<NVGT_OUT>/ (replacing the previous copy). Players who need to override the version the updater reports can do it at runtime through the main menu's "change game version" option, which assigns to the same variable; that override is transient and resets to the compiled-in value on next launch.
...
These are known shape-of-the-code issues. None are urgent — none are breaking anything today — but each gets more expensive to address the longer it's left alone. Don't proactively "fix" them; just be aware they exist when the related area comes up for other reasons.

- **No data-file versioning in info.sif files.** Every authored file (character, weapon, shield, npc, map) is a flat key=value list with no version stamp. Missing keys fall back to engine defaults, which is why old content keeps loading after new fields are added — that part is fine. The hole opens the day you change what an *existing* default means (e.g. flipping a default bullet speed from 400 to 600): every old file that omitted the key silently shifts behavior, with no marker saying "this content was authored against vN.M defaults." Future fix shape: add a `version=X.Y` line at the top of each file, parsed and either stamped on write, version-gated on read, or simply warned about when the file is older than the engine.
...
- **Dual-length encoding is non-obvious and bug-prone.** The "z fields are inserted *inside* the coordinate block, not appended at the end" rule lives only in the Stability rules section above. Every new spanning entity has to remember both the 2d length and the 3d length (and for entities with min/max y, also the legacy single-y topdown variant — so three lengths). A forgotten variant silently breaks one of: backward compatibility, 3d support, or topdown support. Future fix shape: a `parse_sd_line(sd, expected_2d, expected_3d_singley, expected_3d_dualy)` helper that centralizes the length check and emits a clear error on mismatch, so each entity's read function only has to declare its three lengths once.

- **Sound lookups fail silently.** `get_pack_sound("...")` / `get_map_sound("...")` returning no match plays nothing. This is *deliberate* on the player side — a missing clip should never spam a screen reader during gameplay — but it makes authoring brutal, since a typo in a clip path is indistinguishable from a feature that simply has no sound. Future fix shape: a dev-only debug log gated behind `!SCRIPT_COMPILED` or a `debug_sounds` flag that writes failed lookups to a log file. Silent in shipped builds, loud when running the uncompiled script.

- **`remove_platform()` uses array indices, which go stale the moment any other platform is removed.** Entities that own backing platforms (lifts, moving platforms, passages, walls, staircases, conveyor belts, vanishing platforms, floor breakers, hazard side-ledges) store the integer index returned from `spawn_platform()` at spawn time. `remove_platform()` calls `platforms.remove_at(id)`, which compacts the global array — so every other entity's still-stored id past that point now points to the wrong platform. The bug stays hidden during normal gameplay because individual entity destructions usually only fire one removal at a time, but `destroy_all_lifts` / `destroy_all_moving_platforms` / `destroy_all_passages` all fire from `destroymap()` in sequence and each one removes by stale index — every removal in `destroy_all_lifts` shifts the array out from under `destroy_all_moving_platforms` and `destroy_all_passages`, which then take down random unrelated platforms (often user-placed floor tiles). Internal reverse-order sorting within each function only fixes drift *within* that function, not across the chain. Future fix shape: either (a) skip the per-entity `remove_platform()` calls in those three `destroy_all_*` functions entirely (matches the design rule that `destroymap` doesn't touch platforms — orphan tiles persist until the next `clearmap()` which always runs before `load_map`), or (b) replace the integer-index API with stable handles (e.g. a monotonically increasing id that `remove_platform` resolves by scan rather than by array slot) so stored ids survive any number of removals.

- **No tests or linter — the data layer is unverified.** The NVGT compiler catches syntax errors and nothing else. There is no automated check that `read_<entity>` and `write_<entity>` agree, that every key in an authored `info.sif` is one the parser actually recognizes, or that a parsed map contains as many entities as the file had non-blank lines. Every regression has to be noticed by the dev replaying the game. The three deferred concerns immediately above (versioning, parser fallthrough, dual-length encoding) all share the same root: silent failure in a data-driven layer with no validation pass. Future fix shape — a small test-suite directory (e.g. `build/checks/`) with three independent scripts: (a) a **map round-trip smoke test** that loads every `data/maps/decompiled/<name>/info.sif`, immediately writes every parsed entity back out, and diffs against the original — empty diff = read/write are symmetric; (b) a **strict-parse mode for the map loader** gated behind `!SCRIPT_COMPILED` that compares line count vs spawned-entity count and logs unparsed lines; (c) an **info.sif schema check** that walks every authored file under `sounds/main/equipments/**/info.sif` and `sounds/builder/kombat/npc/**/info.sif`, lists the keys present, and flags any the parser doesn't recognize (this would have caught the `hit_and_run` → `hit and run` migration automatically). Wire the suite into `build/tools.py`'s release pipeline so a broken release can't ship.
