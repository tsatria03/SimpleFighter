...
Each top-level folder in sounds/ is a swappable pack selected via the soundpack global (default is the only one in this checkout). All audio lives inside a pack — there are no loose clips at sounds/ root. Per-character rotation clips (and other UI sounds) belong inside characters/<character>/general/ or the matching pack subfolder, not at the root.

Inside a pack:

- characters/<character>/ — per-character bundle. data/main.sif is the stat block parsed by charparse() (fields: weapon type, weapon type2, attack, defence, points, fall modifier, health, stamina, kills, lives, level, level modifier, experience, experience modifier, experience required); data/attacks.sif and data/bodyparts.sif are flat one-token-per-line word lists used to randomize attack/bone-damage flavor text. general/, map/, and the bare folder hold the sound clips that get_pack_sound("characters/<char>/...") resolves against. There is no sound-name list — clips are discovered by glob, so adding a clip is the wiring.
- equipments/shields/<name>/info.sif — 43 shields. key=value per line. Fields: defence, wear mode (0/1), weight, shield strength, unlock level. Sibling clips: draw.ogg, wear.ogg, remove.ogg, hit.ogg, break1.ogg, break2.ogg.
- equipments/weapons/<category>/<name>/info.sif — 271 weapons across archery/, artillery/, explosive/, melee/. Common fields: damage, fire mode (0=single / 1=auto), x range, y range, z range, bullet speed, repeat time, spam time, weight, ammo, loaded ammo, max ammo, unlock level, stun mode. The category folder name must match weapon type in a character's info.sif; the leaf folder name must match weapon type2. Sibling clips vary by weapon (draw, fire, hit, loop1..6, reload1..3, rico, empty, ping, block1..3, ref, on/off).
- kombat/npc/<group>/<name>/info.sif — 187 NPC types across animals/, bosses/, helpers/, humans/, robots/, specials/, zombies/. Stat block fields:
...
- keyboards/<theme>/, menus/<theme>/, misc/, objects/<entity>/, soundtracks/ — theme- and entity-keyed audio resolved by get_pack_sound("...") glob lookups from the engine.
...
Sound assets are looked up via get_pack_sound("...") / get_map_sound("...") with glob patterns like characters/<chartype>/map/*camclear* or objects/walls/<wallname>/*death*. The chartype, menutype, keyboardtheme, soundpack strings drive theme selection.
