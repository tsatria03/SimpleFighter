...
4. **Dispatcher machinery. [DONE 2026-09]** In map_parser.nvgt after the kill block: `move_entity(type, destx,desty,destz)` (16-branch dispatch, keyword `time_bomb` -> move_timebomb, `jet` -> move_security_jet, `camera` -> move_camera; returns bool), `is_movable_type(type)` (16-keyword membership), `dest_in_bounds(x,y,z)` (shared bounds check, z only on 3d). gop rewrite (drop NPC branch + carry-transport) MOVED to Section 5 so gop + its /go caller change together (no transient gap). Braces 237/237; all 16 move_* resolve.

5. **Command handler + gop. [DONE 2026-09]** command_parser.nvgt /go rewritten: player form (arg count 3 on 2d / 4 on 3d) -> gop; type form (4 on 2d / 5 on 3d) -> normalize_buildtype -> is_movable_type guard ("You can't move a X.") -> dest_in_bounds ("...out of map boundries.") -> move_entity -> "Moved the X to A, B[, C]." (int coords) + char *move* cue / "There's no X to move." Gated (!map_is_compiled||trusted). map.nvgt: added `carry_transport()` (snap the ridden machine — invehicle/onbike/inplain, moveable==true — to me.x/y/z; aircraft no sound update) + rewrote gop player-only (dropped NPC + unknown branches; entity/id params kept-but-ignored for existing callers; uses dest_in_bounds; calls carry_transport). Braces map.nvgt 207/207, command_parser 406/406.
6. **Docs + changelog. [DONE 2026-09]** commands.txt /go entry rewritten (unified object syntax, 16-type list, transport-carry, gating; 835 chars). One 15.0 changelog entry at top of block (718 chars). 15.0 now 18 entries (cap 20).

**FEATURE COMPLETE (all 6 sections built & shipping 15.0, 2026-09).** /go moves any of 16 single-point types by keyword (newest) + carries riders both directions. FOLLOW-UP NOW BUILT (2026-09): `carry_transport()` also called from `move_player_axis` (axis commands setx/addx/etc.) and the 4 player-knockback sites (weapon.nvgt me.x=tx x2, bullet.nvgt me.x=tx x2) -- so ANY self-move keeps you aboard, not just /go. 6 call sites total; own 15.0 changelog entry. Drop this plan from the active backlog once 15.0 releases.
