...
**How to apply:** End each response that performed edits with a short "Files changed:" list covering every file written, edited, or deleted that turn — including doc files (changelog, .tp, readme) and config (keyboard.ini), not just code. One line per file. Bare filenames only (e.g. savefuncts.nvgt, game_menu.nvgt) — never directory paths, even partial ones; the dev knows where everything lives. Related: [[feedback_confirm_before_implementing]].
