...
description: "Rules for writing sf/docks/builder/*.tp help topic files — never reference engine internals, describe observable behavior only, keep filenames flat."
metadata: 
  node_type: memory
  type: feedback
...
Rules for writing or editing `.tp` help topic files in `sf/docks/builder/`:
...
- **Filenames must stay flat** — no nested folders — because helpread() strips the `docks/builder/` prefix and `.tp` for the window title.
- **New topics appear automatically.** `/help` (the `hp`/`help` command in `command_parser.nvgt`) scans `docks/builder/*.tp` (under `sf/`, cwd-relative) via `find_files` and lists every topic — no `menu.nvgt` wiring is needed to make a new topic show up. (Verified 2026-06; an older note claimed manual wiring was required.)
...
**Why:** .tp files are read by players and map authors directly from the in-game help menu. Leaking internal names into them is confusing to non-devs and creates maintenance debt when internals are renamed.

**How to apply:** Any time a .tp file is being written or edited, verify no function names, file names, or variable names appear in the prose. If something can't be described without naming code, shorten the explanation instead.
