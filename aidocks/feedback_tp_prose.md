...
- **Mark a field's optionality inline in its label** with a parenthetical after the field name: `(text, optional)`, `(optional number)`, `(required)`, etc. — e.g. `On command (text, optional). ...`, `Cooldown (optional number). ...`, `Command (required). ...`. This is the established topic convention (switches.txt, macros.txt, menu_inputs.txt). It is the help-topic counterpart of the build-form caption "leave blank to skip" wording ([[feedback_builder_form_optional_labels]]) — the topic uses the parenthetical, the live form caption uses "leave blank to skip"; both are correct in their own place.
