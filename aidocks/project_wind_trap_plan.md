...
- **§3 — Map I/O + wiring.** `read_wind` / `write_wind` (line above), `map_parser` dispatch on the `wind` token, `destroy_all_winds()` in `clearmap`, AND an effect-space registration: a loop over `winds` in `apply_effect_pools()` (`effect_space.nvgt`, mirroring fire's line ~94) so `windpool` gets environmental audio like `firepool` — no null guard (compacted array).
