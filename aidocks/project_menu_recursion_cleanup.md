...
**Tier 4 pass 1 DONE (2026-08).** Investigation: death funnels into FOUR near-duplicate "You have died. Try again?" prompts, all called from inside `game()`'s `while(!exit_game)` loop (game.nvgt:30): `playerdeath()` (checkpoint.nvgt:46, main health death via `checkdeath`), `show_death_prompt()` (charfuncts.nvgt:161, bone/command/aircraft/turbulence), inline in `fireloop` (fire.nvgt), inline in `hazardcheck` (hazard.nvgt). Each has retry paths (yes/escape → rebuild in place, loop continues — CORRECT, untouched) and a give-up path (no → menu call, the bug). Fix: every give-up branch now sets `exit_game = true;` instead of calling `gamemenu()`/`mapmenu()` — `game()` breaks and returns to its caller, which re-shows the right menu on its own (verified all game() callers: map plays at map_menu 689/721/730 → `continue` → mapmenu loop; arena plays at game_menu 128/374 `game();clearmap();return;` → gamemenu loop). `was_arena` no longer needed for the give-up menu choice (caller decides) but stays for the retry paths. Only works because Tier 2 + G1 made those callers loops. Sites: checkpoint 118, charfuncts 180, fire 127, hazard 117. Verified: no gamemenu()/mapmenu() left in the four handlers. **DEV TESTED — works.**

**Tier 4 pass 2 (safety guard) DONE (2026-08).** Added `if(exit_game) return;` in `game()` right after the `if(freezgame==false){...}` per-frame check block closes (game.nvgt, before `apply_effect_pools()`). All death triggers (checkdeath/hazardcheck/fireloop/turbcheck/commands) run inside that block, so on a give-up death the frame now exits immediately instead of running apply_effect_pools/game_input/stat-math on the cleared map. NEEDS DEV COMPILE+TEST.

**Remaining followup (do LATER):**
- *Consolidation:* the four death prompts (checkpoint `playerdeath`, charfuncts `show_death_prompt`, inline in fire `fireloop` and hazard `hazardcheck`) share the menu but each has death-cause-specific teardown BEFORE it (checkpoint: shield reset + *death* cry + *fall* + reset_move_stats; charfuncts: teardown done by its caller `bonecheck`; fire: destroy fire sound + *hit* + null entity; hazard: destroy hazard sound + *fall* + remove 2 platform tiles + null entity), and differ on arena-awareness (checkpoint/charfuncts branch on `was_arena`→arena_death_retry vs load_map; fire/hazard are map-only). Shareable core = only the menu + retry/give-up branch logic (parameterized by was_arena), with each site keeping its own teardown+clearmap before calling it (the pattern `show_death_prompt` already uses). NOT a straight merge. NOT YET DONE.
  - *Fire↔hazard inconsistencies (both FIXED 2026-08):* (1) fire's "yes" retry (`mres==0`) was missing `bonehealth=0; broken_bones.delete_all();` that its escape retry had — added. (2) fire ignored checkpoints while hazard honored them — fire now wraps its death in `if(last_checkpoint !is null) restore_last_checkpoint(); else {...}` like hazard (dev: "fires basically behave similar to those"). So fire is now checkpoint-aware; a future consolidation can treat fire and hazard's death structure as matching.
