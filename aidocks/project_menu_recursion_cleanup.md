...
**Tier 4 pass 1 DONE (2026-08).** Investigation: death funnels into FOUR near-duplicate "You have died. Try again?" prompts, all called from inside `game()`'s `while(!exit_game)` loop (game.nvgt:30): `playerdeath()` (checkpoint.nvgt:46, main health death via `checkdeath`), `show_death_prompt()` (charfuncts.nvgt:161, bone/command/aircraft/turbulence), inline in `fireloop` (fire.nvgt), inline in `hazardcheck` (hazard.nvgt). Each has retry paths (yes/escape → rebuild in place, loop continues — CORRECT, untouched) and a give-up path (no → menu call, the bug). Fix: every give-up branch now sets `exit_game = true;` instead of calling `gamemenu()`/`mapmenu()` — `game()` breaks and returns to its caller, which re-shows the right menu on its own (verified all game() callers: map plays at map_menu 689/721/730 → `continue` → mapmenu loop; arena plays at game_menu 128/374 `game();clearmap();return;` → gamemenu loop). `was_arena` no longer needed for the give-up menu choice (caller decides) but stays for the retry paths. Only works because Tier 2 + G1 made those callers loops. Sites: checkpoint 118, charfuncts 180, fire 127, hazard 117. Verified: no gamemenu()/mapmenu() left in the four handlers. NEEDS DEV COMPILE+TEST.

**Followup passes (do LAST, after pass 1 is committed):**
- *Safety guard:* optionally add `if(exit_game) return;` right after the death-check calls in `game()` so the current frame doesn't finish its remaining per-frame checks on the cleared map. Old code already ran those post-menu, so pass 1 is no riskier without it; this just makes it airtight.
- *Consolidation:* the four death prompts are copy-pasted. Extract one shared `death_prompt(was_arena)` and have all four call it. Pure refactor, separate from the recursion fix.
