...
- *Consolidation:* the four death prompts (checkpoint `playerdeath`, charfuncts `show_death_prompt`, inline in fire `fireloop` and hazard `hazardcheck`) are copy-pasted. Extract one shared `death_prompt(was_arena)` and have all four call it. Pure refactor, separate from the recursion fix. NOT YET DONE.
