...
**G1. `load_map` error jumps — `map_parser.nvgt` 216, 234, 269, 342 (4).** On a load failure these `dlg("Error...")` then call `mapmenu()` and **fall through** (216 continues into the parse; 234 continues into `parse_map_meta` on bad data). Real bugs. Should abort the load and return a failure to the caller (which shows the menu). `load_map` is called from many places, so returning cleanly matters.
**G2. `shield_parser.nvgt` 38 — `mainmenu()` on parse error (1).**
**G3. `character_parser.nvgt` 28 — `mainmenu()` on parse error (1).**
Both are fatal-parse paths jumping to the main menu; verify context and convert to a clean abort/return.
