...
**Added (2026-09) to npc_apply_fall_impact:** a combat-log line `round(dmg,0) + " damage to " + subtype + ", falling down."` (mirrors the player's fall log), and — if the NPC SURVIVES the landing (health > 0) — its `*hurt*` clip is played at its position + `react_to_hit` (same pattern as a weapon hit); a lethal fall leaves the death cue to handle the sound. `kombatlog` and the `hurtsound` field were already available in npc.nvgt.

**Push-kill exception (2026-09, folded into the same changelog entry): a fall the PLAYER caused with a push DOES pay out.** New npc fields `pushed_by_player` + `push_timer` + `fall_by_player`. Set `pushed_by_player`+restart `push_timer` at the 4 PLAYER push sites: weapon.nvgt:845/859 (player melee/weapon, always player) and bullet.nvgt:285/309 (only when `bullets[i].firer_npc == -1` = player-fired), each guarded by nonzero push. In npc_fallcheck at fall-START, if `pushed_by_player && push_timer.elapsed < npc_push_credit_ms` (=500ms; a push and the fall it triggers are ~1 frame apart) → set `fall_by_player = true`, then clear `pushed_by_player`. npc_apply_fall_impact then does `killed_by_fall = !fall_by_player` on a lethal fall (so a player-pushed fall does NOT suppress XP), and clears `fall_by_player` on the landing. So: unassisted walk-off/spawn fall = no XP; player push-to-death = full XP + defeated credit. Kills always count.

**Also added (2026-09, SEPARATE changelog entry): no XP for a FALL-caused kill.** New `killed_by_fall` npc field, set true when the fall damage brings health <= 0; the death handler (npc.nvgt ~1914) gates the XP award + its "experience gained / Defeated" combat-log line on `!killed_by_fall`, while `kills += 1` stays UNCONDITIONAL (dev's call: it still counts as your kill, you just don't get the XP payout since you didn't land the blow). The flag is CLEARED on a lose-life revive (currentlives>=2 branch) so a survived near-fall can't stale-withhold XP from a later real kill. Note: this also means push-knocking an enemy off a ledge to its death yields no XP (the fall landed the kill) — accepted.
