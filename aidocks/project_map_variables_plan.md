...
2. **[BUILT, pending dev test]** **Builder form + menu wiring.** In map_variable.nvgt: `build_map_variable()` (coordless 2-field form modeled on build_map_timer — name box whitelisted to letters/digits/underscore, value box; validates name via the shared `map_variable_name_error` and value via `is_number_token` which accepts random()/negatives/decimals; writes + reloads), `write_mapvar()` (appends `mapvar name value`, value stored RAW so random() re-rolls each load), and the shared `map_variable_name_error()`. `normalize_buildtype` gets `map_variable`->`mapvar` (mapfuncts.nvgt). map_menu.nvgt: dispatch `if(buildtype=="map_variable") build_map_variable();`, plus the misc row in BOTH entry_names ("map variable") and entry_ids ("map_variable") alphabetized between map_timer and timed_text, plus "map_variable" added to converted_3d (so it survives the 3d filter; not in excluded_topdown, so all three modes show it; the empty "all" category auto-aggregates + sorts, so it appears there too). Test: build a variable via the form, then build an element referencing it.
