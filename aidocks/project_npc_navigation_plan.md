...
- **§2 — Perpendicular sidestep probe.** When blocked on the primary axis toward the target and making no progress, try a perpendicular step toward the target onto a SUPPORTED cell before giving up — the real "go around a wall," most visible in 3d (step along y around an x-blocking wall). Higher risk (watch for new oscillation); separate + tested alone.
