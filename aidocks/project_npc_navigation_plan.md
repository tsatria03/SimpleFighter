...
## STATUS: Tier 2 BUILT & shipping in 15.0 (all 3 sections). Confirmed working 2d + 3d. Tier 3 (real pathfinding via NVGT's built-in `pathfinder`) remains a future option.

## ALSO BUILT (2026-09): CLIMB-FIRST. Greedy AI walked toward the target's x along the ground and past a staircase, ending up stuck directly below an elevated target (couldn't climb there). Fix in npcloop, right after the §2 suppression and BEFORE move_x: for a non-flyer, if a valid upward climb toward a higher target is available at the current cell, suppress horizontal intent that tick so it climbs first. 2d: `if(dy_intent > 0 && gmt(x,y+1) is solid non-air non-wall) dx_intent = 0;`. 3d: `if(dz_intent > 0 && platform_supports(x,y,z+1)) { dx_intent = 0; dy_intent = 0; }`. Horizontal resumes once at the target's height (no climb left). So an NPC now takes a staircase/ladder to reach a raised player WITHOUT needing a wall built under the platform to funnel it. Confirmed 2d; 3d branch mirrors it (3d staircases are solid platform columns). Trade-off: slightly robotic climb-then-walk; a dead-end staircase toward-but-not-to the target could get climbed pointlessly. Docs (npcs.txt obstacle-nav note) + changelog nav entry updated.
...
- **§2 — Perpendicular sidestep probe. [BUILT + REVISED — awaiting playtest.]** `npc_sidestep(i,tx,ty,terrain_off)` (3d-only, non-flyer) + per-NPC `sidestep_dir` AND `sidestep_axis` (committed wall-follow direction + which axis). Two bugs fixed after first build: (1) §1 was REVERSING onto solid ground (x bounce) so `moved` was always true and the sidestep never fired — changed the blocked-behavior in move_x/move_y to plain HOLD (no reverse); (2) normal pursuit on the sidestep axis undid the sidestep (1-tile oscillation) — now while committed the code SUPPRESSES pursuit on the committed axis, and de-commit is driven by real DISTANCE progress toward the target (Manhattan, `ss_pre_dist`/`ss_post_dist`), not the moved flag. Result: it commits an axis+direction, follows the wall to its end, then resumes normal pursuit. Also made `npc_cell_supported` floor-aware (minz/miny boundary counts as support) so §1/§2 work on boundary-floor maps. Local wall-following, not pathfinding (edge-to-edge walls / far-side openings still trap it).
