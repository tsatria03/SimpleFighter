...
- **When opening a new version block** (adding the first entry under a `New in X.Y.` header that didn't exist before), also bump `src/includes/version.nvgt`'s `version = "X.Y"` constant to match. The changelog and the in-script version are two halves of the same shipped artifact; opening a new version block in only one of them leaves the codebase in an inconsistent state. The bump is a one-line edit and should land in the same change that opens the version block.
