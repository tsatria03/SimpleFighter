...
- **Order within a version stays reverse-chronological** (newest entry at the top of that version's block), matching the existing file. New entries go at the top of the block, not the bottom. **Order by when the change was made, NOT by grouping related entries together.** Even two entries for the same feature go in time order — if a follow-up (e.g. wiring the new entity into the spier) came after the feature's builder entry, the follow-up sits ABOVE the builder entry, not tucked beneath it for tidiness. (Learned 2026-08 on the wind trap: I placed the "added winds to the spier" entry just under the "you can now build winds" entry to keep them together; the dev moved it to the top because the spier change came later. Strict reverse-chron beats feature-grouping.)
