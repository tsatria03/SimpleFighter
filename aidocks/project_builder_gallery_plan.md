...
- bikes (own/misc): beacon, bell, change, crash, death, hurt, plummet, radar, speed, start, stop, stuck, turn [hurt, beacon, death, bell, change, speed, radar, turn, crash, start, stop, stuck] → plummet still untapped. 9 misc extras exposed 2026-09-01: Ctrl+B bell, Ctrl+C change, Ctrl+S speed, Ctrl+D radar, Ctrl+U turn, Ctrl+R crash, Ctrl+T start, Ctrl+P stop, Ctrl+X stuck (bike form + gallery + bikes.txt); all 9 played in bike.nvgt. **plummet is a SHIPPED-BUT-UNWIRED clip**, not dead: the character long-fall cue (fall+plummet, drops >=8 tiles) is suppressed when onbike (map.nvgt:970/990 `&& !onbike`) and the bike's own misc/plummet was meant to play there but never was. Pending decision: wire the onbike fall branch to play misc/*plummet* (2d + 3d) THEN expose it, vs expose-only, vs leave. Verified form key-safety against form.nvgt: while a list is focused the form only consumes Ctrl+F (find), Ctrl+G (go-to, if enabled), Ctrl+A (multiselect); Ctrl+C/X/V are input-field-only — so all 9 bike Ctrl-letters are collision-free.
