...
- `items`: DONE (structure approved) — the special **two-step** type. Unlike every other type, items nest a level deeper on disk: `items/<category>/<variant>/` where category is health / other / stamina. So the gallery's `items` type gets an EXTRA menu level (pick health/other/stamina, then the variant list). DONE 2026-09-01. Map: Space get, Ctrl+L loop, Ctrl+H break, Ctrl+D drop, Ctrl+E fire, Ctrl+I hit — exposed in item form + gallery + items.txt (build-form audition line only; the item-ZONE form was left as-is). The 5 item-SPECIFIC clips (healstart/healstop on other/healing_cast; scanstart/scanning/scanstop on other/health_scanner) were deliberately SKIPPED (dev decision 2026-09-01) — they'd say "no X sound" on ~every item. `place` is a DEAD file (referenced nowhere) — excluded.
...
- `npc`: two-step **and** one extra folder level. Path is `npc/<category>/<subtype>/general/*<clip>*` — pick a category, then a subtype, then preview. 7 categories: animals, bosses, helpers, humans, robots, specials, zombies. Each subtype folder also has a `data/` folder (info.sif) — skip it, clips are under `general/`. DONE — all 11 clips exposed 2026-09-01 in npc form + gallery + npcs.txt (new audition section). Full map: Space hurt, Ctrl+L taunt, Ctrl+H death, Ctrl+N spawn, Ctrl+S step, Ctrl+K hit, Ctrl+E heal, Ctrl+U launch, Ctrl+T tel, Ctrl+I life, Ctrl+R remove. All verified played in npc.nvgt (spawn:260, step:304, remove:665 helpers-only, heal:1081/1088, launch:1134, tel:1150/1154, hit:1519+, life:1591, death:1602). Category caveats stand (remove=helpers-only; hit/step absent from some categories) — gallery/form say "no X sound" there. Step-preview does NOT fall back to the character step clip (dev decision 2026-09-01) even though in-game `use steps=auto/player` can borrow the surface step; the audition just plays the npc's own *step* or announces absence.
...
- `doors`: main `*loop*`; Ctrl+M `*move*` ("move"); Ctrl+C `*close*` ("close"); Ctrl+O `*open*` ("open"); Ctrl+D `*death*` ("death"); Ctrl+H `*hurt*` ("hurt"); Ctrl+N `*deny*` ("deny"); Ctrl+R `*grant*` ("grant"). NOTE: the door FORM has no bare-Space clip — it puts loop on Ctrl+L — but the gallery promotes the ambient `loop` to Space per the Space=main rule (loop is the Space/main clip everywhere else). All 8 clips are genuinely used: `hurt` is played from `bullet.nvgt:420` when a weapon damages a destroyable door (the door class itself never plays it, which is why it isn't in door.nvgt). The leaf's `jam` file is referenced NOWHERE in the code (checked 2026-08-31) — excluded as truly unused. **`step` is NOW USED as of 2026-09-01**: doors (and passages) with looping OFF play their `*step*` clip once on entry, mirroring signs (runtime block in `map.nvgt` after the sign block; `door.was_inside` / `passage.was_inside` gate it). Exposed on Ctrl+S in the door form, passage form, and gallery `doors` type (+ doors.txt/passages.txt) on 2026-09-01.
...
- **First-letter jump in the variant browser (added 2026-09-01).** The audio_form's built-in list first-letter/multinav reads `get_characters()` only for the FOCUSED control, and in the tabbed browser those chars didn't reach the content list (the tab panel is the focused control), so typing a letter did nothing — unlike the build forms. Fix: `gallery_callback` now drains `get_characters()` itself each tick and, on a plain letter/digit (no ctrl/alt), moves the ACTIVE TAB's content-list cursor to the next matching variant (wrapping), via `m.item_count` / `m.get_item_id` / `m.set_focused_item(cand, true)`. It strips the prefix shared by EVERY variant in the tab, but ONLY when doing so leaves alphabetic, varied first letters (walls: dirt/brick/bank → strip "wall", so D lands on walldirt, matching wall.nvgt:341). Numbered prefix-sharing sets — doors (door10, door100) and switches (switch2, switch3) — leave DIGITS after the prefix, so the strip is skipped and they keep their raw first-letter nav unchanged (dev flagged 2026-09-01 that only walls needed the strip). Tabs with no shared prefix (platforms, npcs) match the raw first letter. The jump moves the cursor via a NEW `form_menu.navigate_to(index)` method (added 2026-09-01) rather than `set_focused_item`, so it plays the panned menu click + speaks with position like arrow nav — a plain `set_focused_item` is silent about the click because programmatic cursor moves don't fire the audio_form's `event_list_cursor` (the only thing that triggers `on_form_list_event` → `click_sound`). Draining in the callback (which runs before `form.monitor()`) also stops the built-in nav from double-firing. Plain letters are otherwise unused by the gallery (previews are Space / Ctrl+letter).
...
- items: break, drop, fire, get, healstart, healstop, hit, loop, place, scanning, scanstart, scanstop [get, loop, break, drop, fire, hit] → healstart/healstop/scanstart/scanning/scanstop SKIPPED (item-specific, dev decision), place DEAD. Exposed 2026-09-01: Ctrl+H break, Ctrl+D drop, Ctrl+E fire, Ctrl+I hit.
...
**kombat**
- npc: death, heal, hit, hurt, launch, life, remove, spawn, step, taunt, tel [ALL 11] → none (fully exposed 2026-09-01; remove=helpers-only, hit/step absent from some categories — "no X sound" shown there)
...
**transitions**
- doors: close, death, deny, grant, hurt, jam, loop, move, open, step [loop, move, close, open, death, hurt, deny, grant] → `jam` referenced NOWHERE (dead, do NOT expose); `step` is USED as of 2026-09-01 (loop-off footstep cue on doors AND passages, sign-style) and exposed on Ctrl+S in the door form, passage form, and gallery `doors` type
...
**Misc /play sounds are NOT in the gallery** — they're flat files under `sounds/decompiled/main/misc/` (438 of them), not the category→type→variant→clip folder tree the gallery is built on. They shipped instead as the `/miscsounds` (`/mcn`) command (14.7). The command just calls `miscsoundsmenu()` in `src/includes/main/menus/menu.nvgt`, which enumerates them from the two sources get_pack_sound uses (decompiled folder via find_files + sound_main_pack.list_files() under "misc/"), dedupes, sorts, and shows a flat form_menu. Preview is gallery-style: a `miscsounds_callback` (background_callback) plays the focused clip on SPACE through the preview pool `t`/`prevslot` and stops on cursor move (`mcn_last_idx`); display strips the file extension while the item id keeps the full filename for lookup. No back item — escape closes. Modeled on the old `soundtester()` (git history, includes/menu.nvgt, ~commit 7b66232b). Don't try to fold misc into the gallery's folder structure.
