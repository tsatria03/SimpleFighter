...
**Status: DESIGN, not started. All 8 categories' clip tables are now complete (2026-08-31) — ready to build when the dev gives the go-ahead.** A living spec built up one category at a time. Related: [[project_audio_model]], [[project_sound_assets_layout]], [[feedback_menus_say_canceled]].
...
- **Expose transportation extra clips** in both forms and gallery, same policy. `aircrafts` and `bikes` carry large extra sets (bikes under `misc/`); `vehicles` extras include `hit`, `horn`, `hurt`, `turn` — and `hurt` is already used at runtime (`bullet.nvgt:525`) though the form doesn't preview it. A separate, bigger item: the per-bike `platforms/` ride-surface subtree could become its own gallery section later.
