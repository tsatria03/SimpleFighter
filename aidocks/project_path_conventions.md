...
**Restructure regression fixed 2026-08-30 — the run-from-source restart:** `restart()` in `sf.nvgt` (the in-game "restart game" / restart-after-update paths; callers `menu.nvgt:113` & `:1144`, `updater.nvgt:134`, all `restart("sf.exe")`) relaunches from source via `run("c:\nvgt\nvgtw.exe", scriptid, …)`. Its default `scriptid` was still the pre-restructure bare `"sf.nvgt"`, which no longer resolves (cwd is `sf/`, script moved to `src/`), so restarting errored "File not found: sf.nvgt". Fixed by defaulting `scriptid="../src/sf.nvgt"` (relative to cwd `sf/`, mirroring how the old bare name was cwd-relative). Only affects the source path; the compiled branch uses `appid` ("sf.exe"). A candidate class of bug: any other bare relative path that assumed the old cwd/root.
