...
- **Build-menu forms are covered centrally (fixed 2026-08).** The interactive `build_<entity>` forms (reached from the build menu, and `/build <obj>` with no args) append their line and reload but do NOT call `report_single_map_line` themselves. Tier 1 didn't need them (a form always produces a structurally valid line), but Tier 2 does — a form field can hold an out-of-bounds/bad value. Fixed ONCE in `buildobj` (`map_menu.nvgt`): it records `map_nonempty_line_count()` before dispatch and, if the map grew by a line, calls `report_last_built_line()` (`mapfuncts.nvgt`), which validates the just-appended last line (structural + semantic). So every per-entity checker applies to the build-menu path for FREE — no per-form wiring. (Found when a menu-built `spawnpoint 5 1000` with an out-of-bounds y didn't report; the other write paths — addline/editline, `/build args`, `/rawdata` — were already wired in Tier 1.)
...
- **§1 — Foundation + graduated pilot (walking skeleton).** Build the machinery on the simplest entities so each step adds exactly one new check type (order set by the effort survey below): (a) **spawnpoint** (coords only) → the `entity_semantic_error` second pass wired into the collector, `map_error_block` column support, `is_number` + `coord_in_bounds`; (b) an **easy entity** such as **fall_zone** (ranged box, no names) → adds `range_ok` (min≤max); (c) **checkpoint** (coords + one number + one sound folder) → adds the first `name_exists` disk lookup. Platform and the rest of geometry then confirm it all in §2. These three pilot entities are DONE in §1, so their cluster sections skip them (fall_zone out of §3, spawnpoint + checkpoint out of §8). **STATUS 2026-08: the TRIVIAL, EASY, MODERATE, and INVOLVED tiers are ALL BUILT (50 of 72 entities).** Shared helpers in `map_parser.nvgt`: `is_number_token` (accepts `random(...)`), `coord_in_bounds`, `coord_token_error`, `range_token_error`, `name_exists` (folder glob via `get_map_sound_folders`), **`tile_exists`** (a platform tile, accepting the special no-folder `air` tile — added after finding `name_exists` alone false-flagged air tiles across platform/staircase/slant/vplat/mplat/tile_zone/flux_zone; all now route through it), `is_bool_token`, `dir_id_error` (whole number in a range), **`audio_block_error`** (category musics/sources + clip + volume + pitch, shared by the audio entities) + the `entity_semantic_error(sd, mode, int &out col)` dispatcher; `map_error_block` column support + the second pass wired into `collect_map_file_errors`/`report_single_map_line` + the central `buildobj` build-form hook (`report_last_built_line`). Per-entity checkers live in each entity's own file. Sentinels handled per entity: `none` (most sounds), `any` (tile_zone/flux_zone surface), `random` (projectile_zone). Item/item_zone check a category enum (health/stamina/other) then the item folder within it; menu_zone checks a fixed enum (`menu_zone_ids`). **Incidental fix committed separately:** `read_camera`'s 3d test was `==12` (should be `==13`) — a 3d camera line is length 13, flat is 11 — so 3d cameras were misparsed; fixed + changelogged in 14.4. **Next: the HARD tier (22 entities, the last)** — quoted-text and cross-reference: blockage, el_floor, text_square, timed_text, text_zone, sign, clock, calendar, story_zone, elevator, obscurity_zone, chblocker, cblocker, door, passage, travelpoint, switch, sensor, npc, npc_zone, ttsenemie, effect_space. These need `extract_quoted` shape-mirroring (field positions aren't fixed by index) and new lookup kinds: npc category/subtype content packs, another map's existence (travelpoint via `map_exists`), effect-type + DSP-param contracts, and comma-separated command/setting lists checked against fixed sets. The quoted-text ones (blockage/el_floor/text_square/timed_text/text_zone/sign/clock/calendar/story_zone) are lightest — a quoted string needs almost no semantic check, just skip over it and validate the structural fields around it. **Note: `passage` offers an `air` tile in its form, so route its walkable-tile field through `tile_exists` (not raw `name_exists`)** — the same air-tile trap that hit the construction entities. (A grep of `add_list_item.*"air"` confirms passage is the only remaining hard-tier entity with an air option.) (was originally "platform pilot"; reordered to the spawnpoint→fall_zone→checkpoint ramp after the 2026-08 survey — platform bundles coords+ranges+tile-name+numbers all at once, too big a first bite.)
