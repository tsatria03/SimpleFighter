...
- **§1 — Foundation + graduated pilot (walking skeleton).** Build the machinery on the simplest entities so each step adds exactly one new check type (order set by the effort survey below): (a) **spawnpoint** (coords only) → the `entity_semantic_error` second pass wired into the collector, `map_error_block` column support, `is_number` + `coord_in_bounds`; (b) an **easy entity** such as **fall_zone** (ranged box, no names) → adds `range_ok` (min≤max); (c) **checkpoint** (coords + one number + one sound folder) → adds the first `name_exists` disk lookup. Platform and the rest of geometry then confirm it all in §2. These three pilot entities are DONE in §1, so their cluster sections skip them (fall_zone out of §3, spawnpoint + checkpoint out of §8). **STATUS 2026-08: foundation + the entire TRIVIAL/EASY tier are BUILT** — shared helpers (`is_number_token`, `coord_in_bounds`, `coord_token_error`, `range_token_error`) + the `entity_semantic_error(sd, mode, int &out col)` dispatcher in `map_parser.nvgt`, `map_error_block` column support + the second pass wired into `collect_map_file_errors`/`report_single_map_line` + the central `buildobj` build-form hook; and per-entity checkers for **spawnpoint, fall_zone, airbeacon, air_turbulence, url_source, url_ambience** (the two url entities share one `url_audio_semantic_error` since their field shape is identical). Next: **checkpoint** — the first MODERATE entity, which introduces the `name_exists` disk lookup (its sound folder) — then the rest of the moderate tier. (was originally "platform pilot"; reordered to the spawnpoint→fall_zone→checkpoint ramp after the 2026-08 survey — platform bundles coords+ranges+tile-name+numbers all at once, too big a first bite.)
