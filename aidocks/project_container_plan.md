...
Length: 2d = 15 tokens, 3d = 18. Semantic error keyed on length (like camera 11/13, fire 9/12). Tier 1 lenok + Tier 2 `container_semantic_error` (coords, number ranges, numbers for speed/health/level/xp/dropheight/hovertime, sound-exists-or-none, bool move flags). Alphabetize `container` in the builder traps menu ([[feedback_alphabetize_builder_entities]]) and standard scaffolding (class, containerloop, spawn_/despawn_/destroy_all_/build_/read_/write_/*_semantic_error).

## Build sections (present up front, implement on go-ahead per [[feedback_describe_dont_show_code]])

- **§1 — Core mechanic, placeable + playable.** `container` class + `containerloop()` state machine (below); `spawn_/despawn_/destroy_all_container`; build form (fields below); `read_container` + parser keyword dispatch (map_parser.nvgt) so the line loads; `write_container`; alphabetized traps entry in the build menu (map_menu.nvgt); call `containerloop()` in the game loop; capability-flag hold + fallcheck suppression while caught. Sounds: spawn/loop/catch/flight/hover/drop/remove. NO weapon-destroy, NO reward yet. Testable: place it, it hunts/catches/lifts/hovers/drops you, you take the fall, it removes itself.
- **§2 — Destroy + reward.** Add container to the bullet + weapon-melee (+scadder splash) hit loops (camera `sucams` pattern): hurt on hit, health down, death at zero, camera-style xp reward + "Defeated" log; auto-hit while caught; release-if-caught (fall from current height). Testable: shoot while rolling → dies+xp; shoot mid-lift → released+falls+xp.
- **§3 — Auditions + gallery + docs.** Build-form sound previews (key map above), gallery `containers` type in `gallery_types_for()` (traps), `containers.txt` help topic (incl. "Auditioning sounds" section with the SAME keys), one 15.0 changelog entry.
- **§4 — Map error checker + maps.txt.** Tier 1: register `container` keyword + expected field counts (15 flat / 18 3d) in the error checker's recognized-entity/length table. Tier 2: `container_semantic_error` wired into `entity_semantic_error` dispatch. Update maps.txt to list `container` (alphabetized) among the builder elements. Testable: a malformed container line reports the right error in the Map errors box; maps.txt names it.
