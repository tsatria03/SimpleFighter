...
- **§2 — Reader + writer.** `read_container` + parser keyword dispatch (map_parser.nvgt `sd.length()` gating, 15 flat / 18 3d) so a `container` line loads; `write_container` (append line, camera/fire style). spawn_container already exists (§1).
...
- **§7 — Map error Tier 1.** Register the `container` keyword + expected field counts (15 flat / 18 3d) in the error checker's recognized-entity/length table.
